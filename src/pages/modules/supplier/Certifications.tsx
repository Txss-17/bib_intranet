...
const mockCertifications = [
  {
    ...
    certificateNumber: 'BV-2024-FR-123456',
  ...
  },
  {
    ...
    certificateNumber: 'ECO-2024-ORG-789',
  ...
  },
  {
    ...
    certificateNumber: 'AB-33-2024-001',
  ...
  },
  {
    ...
    issuer: 'AFNOR',
    supplierName: 'AromaPlantes',
    certificateNumber: 'AFN-2023-22716-456',
  ...
  },
  {
    ...
    certificateNumber: 'FT-DE-2024-789',
  ...
  {
    id: '6',
    name: 'BDIH Natural Cosmetics',
    type: 'eco_label',
    issuer: 'BDIH',
    supplierName: 'GreenBeauty',
    certificateNumber: 'BDIH-2024-NC-123',
...
export default function Certifications() {
  ...
  const filteredCertifications = mockCertifications.filter((cert) => {
    const matchesSearch = cert.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      cert.supplierName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const getExpiryInfo = (expiryDate: string) => {
    ...
    if (days < 0) {
      return { text: `Expiré depuis ${Math.abs(days)} jours`, color: 'text-destructive', progress: 0 };
    }
    if (days <= 30) {
      return { text: `Expire dans ${days} jours`, color: 'text-destructive', progress: 10 };
    }
    if (days <= 90) {
      return { text: `Expire dans ${days} jours`, color: 'text-warning', progress: 30 };
    }
    return { text: `Expire dans ${days} jours`, color: 'text-muted-foreground', progress: Math.min(100, days / 3.65) };
  };

  const expiringCount = mockCertifications.filter(c => {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Suivi des certifications fournisseurs et produits
          </p>
        </div>
        <Button>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {expiredCount} certification(s) expirée(s)
                  </p>
          ...
          {expiringCount > 0 && (
            <div className="enterprise-card p-4 border-warning/50 bg-warning/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {expiringCount} certification(s) à renouveler
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Expiration dans les 90 prochains jours
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="enterprise-card p-4">
          ...
          <p className="text-2xl font-semibold text-foreground mt-1">{mockCertifications.length}</p>
        </div>
        <div className="enterprise-card p-4">
        ...
        <div className="enterprise-card p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">À renouveler</p>
          <p className="text-2xl font-semibold text-warning mt-1">
            {mockCertifications.filter(c => c.status === 'pending_renewal').length}
          </p>
        </div>
        <div className="enterprise-card p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">Expirées</p>
          <p className="text-2xl font-semibold text-destructive mt-1">{expiredCount}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Rechercher par nom, fournisseur ou émetteur..."
            value={searchQuery}
      ...
      <div className="space-y-3">
        {filteredCertifications.map((cert) => {
          const typeInfo = certificationTypes[cert.type as keyof typeof certificationTypes] || certificationTypes.other;
          const expiryInfo = getExpiryInfo(cert.expiryDate);
            ...
            <div key={cert.id} className="enterprise-card p-4">
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        <span>
                          {format(new Date(cert.issueDate), 'd MMM yyyy', { locale: fr })} → {format(new Date(cert.expiryDate), 'd MMM yyyy', { locale: fr })}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="w-24">
                    <Progress value={expiryInfo.progress} className="h-1" />
                  </div>
