...
const GatewayOutbound = ({ senderName = '', subject = '', message = '', messageRef = '', sentBy = '', sentByName = '', sentByPosition = '', ccList = [] }: Props) => (
  <Html>
    <Head />
    <Preview>{subject || 'Message Brand in a Box'}</Preview>
      ...
      <Container style={{ maxWidth: '600px', margin: '0 auto', padding: 0, border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
        ...
        <Section style={{ padding: '32px' }}>
          ...
          <Heading style={{ color: TEXT, fontSize: '22px', margin: '0 0 16px', fontWeight: 700 }}>
            {subject || 'Message Brand in a Box'}
          </Heading>
          <Text style={{ color: TEXT, fontSize: '14px', lineHeight: '22px', margin: '0 0 18px' }}>
        ...
        <Section style={{ padding: '20px 32px 28px' }}>
          <Text style={{ color: MUTED, fontSize: '11px', lineHeight: '16px', margin: 0 }}>
            ...
            Cet échange est traité par <strong>Brand in a Box</strong> aux fins de communication
...
export const template = {
  ...
  subject: (d: Props) => d.subject || 'Message Brand in a Box',
